...
def test_the_response_keeps_the_numbers_a_reviewer_needs(
    client: TestClient, service: ScreenerService, uow_factory: Callable[[], UnitOfWork]
) -> None:
    """Withholding `model_verdict` must not mean withholding the evidence."""
    ...
    assert criterion["match_ratio"] == 1.0
...
def test_an_override_without_a_reason_is_rejected(
    ...
    response = client.post(
        f"/candidates/{candidate_id}/override", json={"decision": "advance", "reason": ""}
    )
...
def test_an_invented_decision_is_rejected(
    ...
    response = client.post(
        f"/candidates/{candidate_id}/override", json={"decision": "teleport", "reason": "why"}
    )
